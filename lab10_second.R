...
load_mnist <- function() {
  ...
  train_mat <- load_image_file('train-images.idx3-ubyte')
  test_mat <- load_image_file('t10k-images.idx3-ubyte')
  
  train_mat$y <- load_label_file('train-labels.idx1-ubyte')
  test_mat$y <- load_label_file('t10k-labels.idx1-ubyte')  
  
  trains <<- data.frame(y=train_mat$y, train_mat$x)
  trains$y <<- as.factor(trains$y)
  
  tests <<- data.frame(y=test_mat$y, test_mat$x)
  tests$y <<- as.factor(tests$y)
  }
...
show_digit(as.matrix(trains[1,2:785]))
show_digit(as.matrix(trains[5,2:785]))

# cut the 10k data into tr and ts sets
trainIndex = createDataPartition(tests$y, p = 0.70, list=FALSE)
tr = tests[trainIndex,]
ts = tests[-trainIndex,]
